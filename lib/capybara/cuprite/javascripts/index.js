...
class Cuprite {
  ...
  select(node, value) {
    if (this.isDisabled(node)) {
      ...
      try {
        this.trigger(node.parentNode || parentNode, "blur");
      } catch(e) {
        // best-effort blur after firing changed, which may rerender large parts of the tree, causing node to be unmounted (causing its parentNode to be null) or even that the original parentNode may no longer be able to receive events
      }
